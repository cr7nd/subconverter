...
https://sc.tqq.center:66/sub?target=clash&url=https%3A%2F%2Fdler.cloud%2Fsubscribe%2FABCDE%3Fsurge%3Dss
...
然后将想要的 %TARGET% (即 Clash) 和上一步所得到的 %URL% 填入调用地址中:
https://sc.tqq.center:66/sub?target=clash&url=ss%3A%2F%2FYWVzLTEyOC1nY206dGVzdA%3D%3D%40192%2E168%2E100%2E1%3A8888%23Example1
...
然后将想要的 %TARGET% (即 Clash) 和上一步所得到的 %URL% 填入调用地址中:
https://sc.tqq.center:66/sub?target=clash&url=ss%3A%2F%2FYWVzLTEyOC1nY206dGVzdA%3D%3D%40192%2E168%2E100%2E1%3A8888%23Example1%7Cvmess%3A%2F%2FeyJ2IjoiMiIsInBzIjoidm1lc3MtcHJveHkxIiwiYWRkIjoiZXhhbXBsZS5jb20iLCJwb3J0Ijo0NDMsInR5cGUiOiIiLCJpZCI6IjEyMzQ1Njc4LWFiY2QtMTIzNC0xMjM0LTQ3ZmZjYTBjZTIyOSIsImFpZCI6NDQzLCJuZXQiOiJ3cyIsInBhdGgiOiIvdjIiLCJob3N0IjoiZXhhbXBsZS5jb20iLCJ0bHMiOiJ0bHMifQ%3D%3D
...
```txt
https://sc.tqq.center:66/surge2clash?link=Surge的订阅链接
```
...
```txt
https://sc.tqq.center:66/sub?target=%TARGET%&url=%URL%&emoji=%EMOJI%····
```
...
接着将所有元素进行拼接：
https://sc.tqq.center:66/sub?target=surge&ver=4&tfo=true&udp=true&emoji=true&exclude=%28%E6%B5%81%E9%87%8F%7C%E5%AE%98%E7%BD%91%29&url=https%3A%2F%2Fdler.cloud%2Fsubscribe%2FABCDE%3Fclash%3Dvmess
...
```txt
https://sc.tqq.center:66/getprofile?name=%NAME%&token=%TOKEN%
```
...
在编辑并保存好 `formyairport.ini` 后，即可使用 `https://sc.tqq.center:66/getprofile?name=profiles/formyairport.ini&token=passwd` 进行调用。
...
3.  **default_url**
    ...
    -   解释：
        ...
        ```txt
        此时订阅链接:
        https://sc.tqq.center:66/sub?target=clash
        等同于:
        https://sc.tqq.center:66/sub?target=clash&url=https%3A%2F%2Fdler.cloud%2Fsubscribe%2FABCDE%3Fclash%3Dvmess
        ```
...
3.  从 URL 链接中获取，判断前缀为 `request`，例如 `https://sc.tqq.center:66/sub?target=clash&url=www.xxx.com&clash.dns=1`
...
```txt
https://sc.tqq.center:66/render?path=xxx&额外的调试或控制参数
```
...
```txt
https://sc.tqq.center:66/getruleset?type=%TYPE%&url=%URL%&group=%GROUP%
```
